...
// Forward declaration from other_code.cpp
     ...
     char *argv[])
{
    ...
    PointCloudT::Ptr cloud_in = get_cloud(); // Original point cloud from other_code.cpp
    PointCloudT::Ptr cloud_tr = get_cloud(); // Transformed point cloud
    PointCloudT::Ptr cloud_icp(new PointCloudT); // ICP output point cloud

    int iterations = 1; // Default number of ICP iterations
    if (argc > 2)
    {
        // If the user passed the number of iteration as an argument (kept for backward compatibility)
    ...
    time.tic();
    std::cout << "\nLoaded cloud from get_cloud() (" << cloud_in->size() << " points) in " << time.toc() << " ms\n" << std::endl;
